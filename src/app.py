...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    while step < MAX_ITERATIONS:
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            print("🧠 [Thought]: Đã nhận Observation từ MCP Server, tiếp tục suy luận bước kế tiếp.")
        else:
            print("⚠️ [REACT AGENT]: Provider trả về phản hồi không hợp lệ.")
            break

    if not trace_logs or trace_logs[-1].get("action_type") != "FINAL_ANSWER":
        final_answer = "Agent đã đạt giới hạn số bước trước khi đưa ra câu trả lời cuối cùng."
        print(f"🏁 [Final Answer]: {final_answer}")
        trace_logs.append({
            "step": step + 1,
            "query": user_query,
            "action_type": "FINAL_ANSWER",
            "thought": "Kết thúc an toàn vì không nhận được phản hồi cuối cùng hợp lệ.",
            "output": final_answer,
            "latency_ms": 0.0
        })
